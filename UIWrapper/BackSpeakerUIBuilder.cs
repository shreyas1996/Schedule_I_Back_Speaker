...
namespace BackSpeakerMod.UIWrapper
{
    ...
    public static class BackSpeakerUIBuilder
    {
        ...
        public static BackSpeakerMainScreen CreateMainScreen(Transform parent, MainScreenConfig? config = null)
        {
            ...
            // Create main layout
            screen.MainLayout = LayoutFactory.CreateMainScreenLayout(parent, config.LayoutConfig ?? null);

            // Create track info component in header
            if (config.ShowTrackInfo && screen.MainLayout != null)
            {
                screen.TrackInfo = ComponentBuilder.CreateTrackInfoComponent(screen.MainLayout.HeaderArea!.transform, config.TrackInfoConfig ?? null);
            }

            // Create content components
            if (config.ShowControls && screen.MainLayout != null)
            {
                screen.Controls = ComponentBuilder.CreateControlsComponent(screen.MainLayout.ContentArea!.transform, config.ControlsConfig ?? null);
            }

            if (config.ShowProgressBar && screen.MainLayout != null)
            {
                screen.ProgressBar = ComponentBuilder.CreateProgressBarComponent(screen.MainLayout.ContentArea!.transform, config.ProgressBarConfig ?? null);
            }

            if (config.ShowTabBar && screen.MainLayout != null)
            {
                screen.TabBar = ComponentBuilder.CreateTabBarComponent(screen.MainLayout.ContentArea!.transform, config.TabBarConfig ?? null);
            }

            // Position main layout
            if (screen.MainLayout != null)
            {
        ...
        public static BackSpeakerPopupScreen CreatePopupScreen(Transform parent, PopupScreenConfig? config = null)
        {
            ...
            if (screen.Popup?.Layout?.ContentArea != null)
            {
        ...
        private static void CreatePlaylistSelectionContent(Transform parent, PopupScreenConfig config)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (listLayout.ContentArea == null) continue;
